const genEntry = require('./compiler');
...
class MpvueEntry {
  ...
  static getEntry(customPaths) {
    ...
    if (typeof customPaths === 'string') {
      paths.pages = resolveApp(customPaths);
    } else {
      Object.keys(customPaths).forEach((key) => {
        paths[key] = resolveApp(customPaths[key]);
      });
    }
    if (!paths.template) {
      paths.template = paths.main;
    }
