...
class MpvueEntry {
  static getEntry(customPaths, customOptions) {
    ...
    // 生成入口
    let entry = genEntry(paths);

    // 监听文件
    if (options.watch) {
      const {
        pages, template, bakPages, bakTemplate,
      } = paths;

      watchFile(pages, () => {
        entry = genEntry(paths, true);
        writeFile(bakPages, JSON.stringify(require(pages), null, '  '));
      });
      watchFile(template, () => {
        entry = genEntry(paths);
        writeFile(bakTemplate, template);
      });
    }

    if (!options.dynamic) return entry;
  ...
  apply(compiler) {
    ...
    compiler.plugin('done', (stats) => {
      if (!stats.compilation.assets['app.json'].emitted) return;
      resetApp(paths);
    });
  }
}
