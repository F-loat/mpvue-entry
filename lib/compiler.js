...
function genConfig(paths, pages, home) {
  ...
  pages.formated.forEach(page => {
    if (page.root) {
      page.subPackage = true
    }
  });
  ...
  app.subPackages = pages.formated
    .filter(page => page.subPackage)
    .reduce((result, page) => {
      const root = page.root ? page.root : page.path.replace(/\/.*$/, '');
      const subPath = page.root ? page.path.replace(page.root, '') : page.path.replace(/.*?\//, '');
