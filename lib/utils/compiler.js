...
function genEntry(paths, cause = 'initial') {
  ...
  if (cause !== 'initial') {
    ...
    const homeMatch = template.content.match(/pages:[^]*\^(.*)['|"]/);

    app.pages = pages.current.map(page => page.path);
    if (homeMatch) {
      const homePage = homeMatch[1];
      const homeIndex = app.pages.findIndex(page => page === homePage);
      app.pages.splice(homeIndex, 1);
      app.pages.unshift(homePage);
    }
